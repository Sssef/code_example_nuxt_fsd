...
> [!NOTE]
> Backend предоставляет только коллекции (списки фильмов, кинотеатров, бронирований) и не имеет эндпоинтов для получения одного ресурса по ID.
BFF берёт на себя подготовку данных для страниц конкретного фильма, кинотеатра, сеанса — фильтрует списки, агрегирует сведения из разных источников (например, для страницы сеанса загружает детали фильма и кинотеатра).
